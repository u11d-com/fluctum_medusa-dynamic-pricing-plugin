...
export const createCartPriceLocksStep = createStep(
  ...
  async (input: CreateCartPriceLocksStepInput, { container }) => {
    ...
    const pricingModule = container.resolve(
      DYNAMIC_PRICING_MODULE,
    ) as DynamicPricingModuleService;
    ...
    const items = (cart.items ?? []).filter(
      (item: CartLineItemDTO) => item.variant_id,
    );
    if (items.length === 0) {
      return new StepResponse(
        { locks: [], expires_at: new Date().toISOString() },
        input.cartId,
      );
    }
    ...
    // Deliberately reads from the DB (not a direct provider call) regardless
    // of `force`. Calling the provider on every force=true lock would make
    // third-party API load scale linearly with checkout traffic (e.g. 10
    // checkouts/sec = 10 provider requests/sec, unbounded by
    // fetchIntervalSeconds), risking rate limits/cost and tying checkout
    // latency to that provider's availability. The scheduled job already
    // refreshes this table on a fixed interval independent of traffic, which
    // bounds staleness predictably; see ADR 0005.
    const spotPrices = await pricingModule.getLatestSpotPrices(materials);
    const spotByMaterial = new Map(
      spotPrices.map((sp) => [sp.material, sp.price]),
    );

    const now = new Date();
    const expiresAt = new Date(
      now.getTime() + options.priceLockDurationSeconds * 1000,
    );
    ...
    const cartCurrency =
      ((cart as unknown as Record<string, unknown>).currency_code as
        | string
        | undefined) ?? "USD";
    ...
    if (cartCurrency.toUpperCase() !== pricingCurrency.toUpperCase()) {
      const rates = await pricingModule.getLatestRates(pricingCurrency, [
        cartCurrency.toUpperCase(),
      ]);
      const rateRow = rates.find(
        (r) => r.to_currency.toUpperCase() === cartCurrency.toUpperCase(),
      );
      if (rateRow) {
    ...
    const rawNum = (v: number) =>
      JSON.stringify({ value: String(v), precision: 20 });
