import { resolvePluginOptions, ConfigValidationError } from "../config";
import type {
...
describe("resolvePluginOptions", () => {
  describe("valid configuration", () => {
    it("accepts minimal valid options and applies defaults", () => {
      ...
      expect(result.priceLockDurationSeconds).toBe(600);
  ...
  describe("invalid materials", () => {
    it("throws when materials is missing", () => {
      expect(() => resolveInvalid({ provider: mockProvider })).toThrow(
  ...
  describe("invalid provider", () => {
    it("throws when provider is missing", () => {
      expect(() => resolveInvalid({ materials: ["XAU"] })).toThrow(
        ConfigValidationError,
      );
    });
  ...
  describe("pricingCurrency", () => {
    ...
    it("throws on non-string value", () => {
      expect(() =>
        resolveInvalid({ ...validOptions, pricingCurrency: 123 }),
      ).toThrow(ConfigValidationError);
    });

    it("throws on 2-letter code", () => {
      expect(() =>
        resolvePluginOptions({ ...validOptions, pricingCurrency: "US" }),
      ).toThrow(ConfigValidationError);
    });

    it("throws on 4-letter code", () => {
      expect(() =>
        resolvePluginOptions({ ...validOptions, pricingCurrency: "USDD" }),
      ).toThrow(ConfigValidationError);
    });
  });
  ...
  describe("currencyConversion", () => {
    ...
    it("accepts a valid currencyConversion block", () => {
      const result = resolvePluginOptions({
        ...validOptions,
        currencyConversion: validConversion,
      });
      ...
      expect(result.currencyConversion?.targetCurrencies).toEqual([
        "EUR",
        "PLN",
      ]);
    });
    ...
    it("throws when currencyConversion is not an object", () => {
      expect(() =>
        resolveInvalid({ ...validOptions, currencyConversion: "static" }),
      ).toThrow(ConfigValidationError);
    });
    ...
    it("throws when currencyConversion.provider is missing", () => {
      expect(() =>
        resolveInvalid({
          ...validOptions,
          currencyConversion: { targetCurrencies: ["EUR"] },
        }),
      ).toThrow(ConfigValidationError);
    });
    ...
    it("throws when currencyConversion.provider is not a function", () => {
      expect(() =>
        resolveInvalid({
          ...validOptions,
          currencyConversion: { ...validConversion, provider: "static" },
        }),
      ).toThrow(ConfigValidationError);
    });
